...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Platinumcoin address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"platinumcoinaddress\"    (string) The new platinumcoin address\n"
            "\nExamples:\n"
    ...
    return CPlatinumcoinAddress(keyID).ToString();
}


CPlatinumcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CPlatinumcoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current Platinumcoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"platinumcoinaddress\"   (string) The account platinumcoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "setaccount \"platinumcoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"platinumcoinaddress\"  (string, required) The platinumcoin address to be associated with an account.\n"
    ...
    CPlatinumcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Platinumcoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"platinumcoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"platinumcoinaddress\"  (string, required) The platinumcoin address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"platinumcoinaddress\"  (string) a platinumcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CPlatinumcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CPlatinumcoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            "sendtoaddress \"platinumcoinaddress\" amount ( \"msg\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"platinumcoinaddress\"  (string, required) The platinumcoin address to send to.\n"
            "2. \"amount\"      (ep, required) The amount in PTCO to send. eg 0.1\n"
...
Value setlimit(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "2. \"platinumcoinaddress\"  (string, required) The platinumcoin address to update.\n"
	    "\nResult:\n"
    ...
    CPlatinumcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Platinumcoin address");
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"platinumcoinaddress\",     (string) The platinumcoin address\n"
            "      amount,                 (ep) The amount in PTCO\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CPlatinumcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CPlatinumcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CPlatinumcoinAddress(address).Get())->second.name);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"platinumcoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"platinumcoinaddress\"  (string, required) The platinumcoin address to use for the private key.\n"
    ...
    CPlatinumcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"platinumcoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given platinumcoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"platinumcoinaddress\"  (string, required) The platinumcoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (ep) The total amount in PTCO received at this address.\n"
            "\nExamples:\n"
    ...
    // Platinumcoin address
    CPlatinumcoinAddress address = CPlatinumcoinAddress(params[0].get_str());
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (ep) The total amount in PTCO received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 PTCO from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" \"0.01000000ep\"") +
            "\nMove 0.01 PTCO timotei to akiko with a comment and funds have 6 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toplatinumcoinaddress\" amount ( minconf \"msg\" )\n"
            "\nSent an amount from an account to a platinumcoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"         (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toplatinumcoinaddress\" (string, required) The platinumcoin address to send funds to.\n"
            "3. amount                  (ep, required) The amount in PTCO. (transaction fee is added on top).\n"
            "4. minconf                 (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            "5. \"msg\"                 (string, optional) A comment used to store what the transaction is for. \n"
            "\nResult:\n"
            "\"transactionid\"          (string) The transaction id. (view at https://blockchain.info/tx/[transactionid])\n"
            "\nExamples:\n"
            "\nSend 0.01 PTCO from the default account to the address, must have at least 1 confirmation\n"
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":\"amount\"  (ep) The platinumcoin address is the key, the numeric amount in PTCO is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CPlatinumcoinAddress> setAddress;
    map<CKeyID, int64_t> mapSend;

    uint64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CPlatinumcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Platinumcoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Platinumcoin address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of platinumcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) platinumcoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"platinumcoinaddress\"  (string) A platinumcoin address associated with the keys.\n"
    ...
    return CPlatinumcoinAddress(inner).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CPlatinumcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CPlatinumcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CPlatinumcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (ep) The total amount in PTCO received by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"address\":\"platinumcoinaddress\",    (string) The platinumcoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (ep) The amount in PTCO. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"fee\": x.xxx,             (ep) The amount of the fee in PTCO. This is negative and only available for the \n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"platinumcoinaddress\",    (string) The platinumcoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (ep) The amount in PTCO. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"fee\": x.xxx,             (ep) The amount of the fee in PTCO. This is negative and only available for the 'send' category of transactions.\n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (ep) The transaction amount in PTCO\n"
            ...
            "      \"address\" : \"platinumcoinaddress\",   (string) The platinumcoin address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (ep) The amount in PTCO\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending PTCO\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending PTCO\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"platinumcoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Platinumcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (ep, required) The transaction fee in PTCO/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (ep) the total PTCO balance of the wallet\n"
