...
#if defined(NDEBUG)
# error "Platinumcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Platinumcoin Signed Message:\n";
...
int64_t GetBlockValue(uint64_t nHeight, uint64_t coinbase, uint64_t nFees)
{
    uint64_t value = 0;

    if ( nHeight <= 1 ) {
	value = 10000000 * COIN;
    } else if ( (nHeight>1) && (nHeight<250000) ) {
	value = 6.25 * COIN;
	...
	value = 50 * COIN;
	value >>= (nHeight / 840000); 	// Platinumcoin: 840k blocks in ~4 years
    }
    return value + nFees;
}

static const int64_t nTargetTimespan = 3.5 * 24 * 60 * 60;	// 3.5 days
static const int64_t nTargetSpacing = 2.5 * 60;			// 2.5 minutes
...
void ThreadScriptCheck() {
    RenameThread("platinumcoin-scriptch");
...
void ActivateTrie(){
	...
	So print error, and exit, if not, platinumcoind will eat all aviable memory and die by oom-killer
	*/
	fBuilding=false;
