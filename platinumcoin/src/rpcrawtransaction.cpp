...
bool DecomposeSig(uint256 hash, vector<unsigned char> sig, vector<pair<uint160, vector<unsigned char> > > &recoveredKeys, vector<uint160> &explicitKeys){
	...
	CPlatinumcoinAddress address(key.GetID());
...
uint160 ValueFromSig(uint256 hash, vector<unsigned char> sig, Object &o, int assumedSigs){
    ...
    for(uint32_t i=0; i < nSigs; i++){
	signs.push_back(Pair("address",CPlatinumcoinAddress(CKeyID(recoveredKeys[i])).ToString()));
    }
    ...
    Object hashes;
    for(uint32_t i=0; i < nHashSigs; i++){
	hashes.push_back(Pair("address",CPlatinumcoinAddress(CKeyID(explicitKeys[i])).ToString()));
    }
	...
	o.push_back(Pair("recovered_adr", CPlatinumcoinAddress(CKeyID(allKeys[0])).ToString()));
	return allKeys[0];
    }
    ...
    o.push_back(Pair("valid", true));
    o.push_back(Pair("recovered_adr", CPlatinumcoinAddress(CKeyID(recHash)).ToString()));

    if(assumedSigs){
    	data[sizeof(data)-1] = assumedSigs;
    	uint160 assHash = Hash160(data,data+sizeof(data));	
        o.push_back(Pair("if_signed_adr", CPlatinumcoinAddress(CKeyID(assHash)).ToString()));
    }
...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry, map<int,int> *mapSigs=0)
{
	...
	in.push_back(Pair("address", CPlatinumcoinAddress(CKeyID(txin.pubKey)).ToString()));
	...
	out.push_back(Pair("address", CPlatinumcoinAddress(CKeyID(txout.pubKey)).ToString()));
        vout.push_back(out);
    }
...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
	    ...
	    "	    \"address\" : \"addr\",(string) Platinumcoin address representation of public key hash\n"
            ...
            "     {\n"
            "       \"value\" : x.xxx,     (ep) The value in PTCO\n"
            "       \"n\" : n,             (numeric) index\n"
            "       \"pubkey\": \"key\",   (string) The public key hash of the input\n"
	    "	    \"address\" : \"addr\",(string) Platinumcoin address representation of public key hash\n"
            "     }\n"
...
Value listbalances(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2 || params.size() < 2)
        throw runtime_error(
            ...
            "2. \"addresses\"    (string) A json array of platinumcoin addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) platinumcoin address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"address\" : \"address\",  (string) the platinumcoin address\n"
	    "    \"ours\"    : true/false,   (bool) is the account belongs to the local wallet\n"
            ...
            "    \"balance\" : x.xxx,        (ep) the account balance in PTCO\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    vector<CPlatinumcoinAddress> vecAddress;
    ...
    if (params.size() > 1)
    {
        ...
        {
            CPlatinumcoinAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid platinumcoin address: ")+input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() < 2 && params.size() > 4 ))
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (ep, required) The key is the platinumcoin address, the value is the PTCO amount\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CPlatinumcoinAddress> setInAddress;
    BOOST_FOREACH(const Pair& input, inputs)
    {
    	CPlatinumcoinAddress address(input.name_);
    	if (!address.IsValid())
    		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Platinumcoin address: ")+input.name_);
    ...
    set<CPlatinumcoinAddress> setOutAddress;
    BOOST_FOREACH(const Pair& output, outputs)
    {
        CPlatinumcoinAddress address(output.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Platinumcoin address: ")+output.name_);
...
Value setuprawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
	    ...
	    "2. \"inputs\"     (string, required) JSON object with input id's as keys and multisig descriptions as values\n"
            ...
            "        2. \"keys\"       (string, required) A json array of keys which are platinumcoin addresses or hex-encoded public keys\n"
            "        [\n"
            "          \"key\"    (string) platinumcoin address or hex-encoded public key\n"
            "          ,...\n"
            "        ]\n"
            "      ]\n"
            "      ,...\n"
	...
	const Array& keys = msigParms.get_array()[1].get_array();
	    ...
	    const std::string& ks = keys[i].get_str();
            CPlatinumcoinAddress address(ks);
	    CKeyID keyID;
	    address.GetKeyID(keyID);
	    keyIDs[i]=keyID;
        }
