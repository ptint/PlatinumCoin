...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, int vout, int unit)
{
...

    {
                    ...
                    if (wallet->IsMine(txout))
                    {
                        ...
                        if (wallet->IsMine(txout.pubKey))
                        {
                            if (wallet->mapAddressBook.count(address))
                            {
                                ...
                                strHTML += "<b>" + tr("To") + ":</b> ";
                                strHTML += GUIUtil::HtmlEscape(CPlatinumcoinAddress(address).ToString());
        ...
        if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
        {
            ...
            strHTML += "<b>" + tr("To") + ":</b> ";
            CTxDestination dest = CPlatinumcoinAddress(strAddress).Get();
        ...
        if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += "<b>" + tr("Credit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += "<b>" + tr("Debit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, -txout.nValue) + "<br>";
                }

                if (fAllToMe)
                {
                    // Payment to self
                    strHTML += "<b>" + tr("Debit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, nDebit) + "<br>";
                    strHTML += "<b>" + tr("Credit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, nCredit) + "<br>";
                }

                uint64_t nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += "<b>" + tr("Transaction fee") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += "<b>" + tr("Debit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += "<b>" + tr("Credit") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, wallet->GetCredit(txout)) + "<br>";
            }
        }

        strHTML += "<b>" + tr("Net amount") + ":</b> " + PlatinumcoinUnits::formatWithUnit(unit, nNet, true) + "<br>";
        ...
        // Message from normal platinumcoin:URI (platinumcoin:123...?message=example)
        ...
        if (fDebug)
        {
...

            {
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                {
                    ...
                    if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].name.empty())
                        strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address].name) + " ";
                    strHTML += QString::fromStdString(CPlatinumcoinAddress(address).ToString());

                    strHTML = strHTML + " " + tr("Amount") + "=" + PlatinumcoinUnits::formatWithUnit(unit, txin.nValue);
