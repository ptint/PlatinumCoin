...
CBlockTemplate* CreateNewBlock(uint160 pubKey)
{
	...
	pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, balance, nFees);
...
bool CheckWork(CBlock* pblock, uint256 hashTarget, CWallet& wallet)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PlatinumcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessBlock(state, pblock))
        return error("PlatinumcoinMiner : ProcessBlock, block not accepted");

    return true;
}

void static PlatinumcoinMiner(CWallet *pwallet)
{
    LogPrintf("PlatinumcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("platinumcoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running PlatinumcoinMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("PlatinumcoinMiner terminated\n");
        throw;
    }
}

void GeneratePlatinumcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PlatinumcoinMiner, pwallet));
}
